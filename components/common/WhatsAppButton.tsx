...
export const WhatsAppButton = React.memo(function WhatsAppButton({
  ...
  if (fixed) {
    return (
      <motion.button
        ...
        className={cn(
          'fixed z-50 text-white rounded-full shadow-2xl flex items-center justify-center group overflow-hidden backdrop-blur-xl border-2 border-white/30',
          'bottom-20 right-4 sm:bottom-6 sm:right-6',
          'w-14 h-14 sm:w-16 sm:h-16',
          className
        )}
        style={{ background: 'linear-gradient(135deg, rgba(37, 211, 102, 0.9), rgba(37, 211, 102, 0.85))' }}
  ...
  return (
    <motion.button
      ...
      className={cn(
        'inline-flex items-center justify-center gap-2 px-6 py-3 text-white font-semibold rounded-2xl transition-all duration-300 shadow-lg hover:shadow-xl relative overflow-hidden backdrop-blur-xl border-2 border-white/30',
        className
      )}
      style={{ background: 'linear-gradient(135deg, rgba(37, 211, 102, 0.9), rgba(37, 211, 102, 0.85))' }}
    >
      ...
      <div className="absolute inset-0 bg-gradient-to-r from-transparent to-white/10 rounded-2xl"></div>
